...
angular.module('owsWalletPlugin.controllers').controller('SellCtrl', function($scope, $log, $state, $timeout, $ionicHistory, $ionicScrollDelegate, $ionicConfig, lodash, coinbaseService, popupService, profileService, ongoingProcess, walletService, appConfigService, configService, txFormatService, externalLinkService) {
...
//      var accessToken = res.accessToken;
      ...
      coinbaseService.sellPrice(coinbaseService.getAvailableCurrency(), function(err, s) {
...
//      var accessToken = res.accessToken;
      ...
      coinbaseService.sellPrice(coinbaseService.getAvailableCurrency(), function(err, sell) {
        if (err) {
        ...
        coinbaseService.getTransactions(accountId, function(err, ctxs) {
          if (err) {
            $log.debug(coinbaseService.getErrorsAsString(err.errors));
            checkTransaction(count, txp);
            return;
...
//      var accessToken = res.accessToken;
      ...
      coinbaseService.sellRequest(accountId, dataSrc, function(err, data) {
        ongoingProcess.set('connectingCoinbase', false);
        if (err) {
          showErrorAndBack(coinbaseService.getErrorsAsString(err.errors));
          return;
...
//        var accessToken = res.accessToken;
        ...
        coinbaseService.createAddress(accountId, dataSrc, function(err, data) {
          if (err) {
            ongoingProcess.set('sellingBitcoin', false, statusChangeHandler);
            showError(coinbaseService.getErrorsAsString(err.errors));
            return;
